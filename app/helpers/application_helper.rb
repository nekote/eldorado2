...
module ApplicationHelper  
  ...
  def rank_for(user)
    return 'Administrator, Suspended' if user.admin && user.banned?
    return 'Administrator' if user.admin
    return 'Suspended' if user.banned?
  ...
  def time_stamp(time, short = false)
    return (time.strftime("%l:%M%p ").downcase + time.strftime("%d%b%y")).squeeze(' ') if short
